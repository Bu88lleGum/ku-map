...
// Floors 0–10
const FLOORS = Array.from({ length: 11 }, (_, i) => i);   // [0,1,2,...,10]
...
export default function App() {
  ...
  // Auto-switch to the first floor of a new route
  useEffect(() => {
  ...
  const floorsWithPath = pathResult
    ? [...new Set(pathResult.path_nodes.map((n) => n.floor))].sort((a, b) => a - b)
    : [];
  ...
  return (
    <div
      className="min-h-screen h-screen flex flex-col bg-[#080c14] text-white overflow-hidden"
    ...
    >
      ...
      <header className="flex-shrink-0 border-b border-slate-800/70 px-5 h-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <div className={`w-1.5 h-1.5 rounded-full ${nodes.length > 0 ? 'bg-emerald-400 animate-pulse' : 'bg-slate-600'}`} />
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {nodesLoading ? (
              <div className="space-y-4">
                <div><SkeletonBlock className="w-16 h-3 mb-2" /><SkeletonBlock className="w-full h-11" /></div>
                <div><SkeletonBlock className="w-12 h-3 mb-2" /><SkeletonBlock className="w-full h-11" /></div>
              </div>
            ) : (
              <div className="space-y-3">
                <NodeSelector nodes={nodes} value={startId}
                  onChange={(id) => { setStartId(id); clearPath(); }}
                  placeholder="Отправная точка" label="Откуда" accentColor="#22c55e" />

                <div className="flex justify-center">
                  <button onClick={handleSwap} disabled={!startId && !endId}
                ...
                <NodeSelector nodes={nodes} value={endId}
                  onChange={(id) => { setEndId(id); clearPath(); }}
                  placeholder="Конечная точка" label="Куда" accentColor="#ef4444" />
              </div>
            )}
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-[#080c14]">
          ...
          {/* ── Floor tabs (0–10, scrollable) ── */}
          <div className="flex-shrink-0 flex items-end gap-0.5 px-4 pt-3 border-b border-slate-800/70 overflow-x-auto">
            {FLOORS.map((floor) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex-shrink-0 px-4 py-2 text-xs font-semibold tracking-[0.1em] uppercase rounded-t-lg transition-all ${
                    isActive
          ...
          {floorsWithPath.length > 1 && (
            <div className="flex-shrink-0 px-4 pb-3 flex items-center gap-2 flex-wrap">
