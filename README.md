...
#### 📍 Система обозначений узлов (Nodes)
- **Лестницы**: Обозначаются одной цифрой (1, 2, 3). Соответствуют номеру этажа.

- **Коридоры**: Двузначные числа (13, 21, 32). Соединяют кабинеты и лестничные пролеты.

- **Кабинеты**: Трёхзначные числа (106, 201, 302). Первая цифра всегда указывает на этаж.

- **Входы**: * 
99 — Главный вход (справа).

98 — Запасной вход (слева).

#### ⚖️ Веса переходов (Weights)
Для реалистичного построения маршрутов установлены разные веса на подъем и спуск:

- **⬆️ Вверх по лестнице**: 15 (требует больше усилий).

- **⬇️ Вниз по лестнице**: 10 (быстрее и легче).

- **🏃 Горизонтальные пути**: рассчитываются автоматически по длине линий в метрах.

#### 🔗 Логика соединений
- **Вертикальные**: Лестницы с одинаковыми координатами на разных этажах связываются между собой (1 <-> 2, 2 <-> 3).

- **Горизонтальные**: Кабинеты привязываются к ближайшим узлам коридоров на своем этаже.

- **Входы**: Узел 99 и 98 на первом этаже соединены с центральной сетью коридоров для доступа в здание.
